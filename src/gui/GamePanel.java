...
public class GamePanel extends JPanel implements KeyListener, ActionListener {
  ...
  private final int SOLUTION_TIME_LIMIT = 150000000;

  private boolean reportMode = false;
  private double reportSolutionTime;
  ...
  public void startThinking(boolean reportMode) {
    waitingForSpace = false;
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource() == animationTimer) {
      if (this.solutionCtr >= this.solutionString.length()) {
        ...
        if (reportMode) {
          parent.resolveTest(reportSolutionTime, progress, boxCount, moves);
        }
      ...
      if (!solutionThread.isAlive()) {
        ...
        if (!reportMode)
          ...
          this.playSolution(solution, 5);
      }
      long elapsedSolutionTime = System.nanoTime() - solutionStartTime;
      reportSolutionTime = elapsedSolutionTime / 1000000000.0;
